...
**Your Answers here:**
1. In dataSet1.R - code: mytickers = c("SPY", "AAPL")
2. #Store only the values of Apple's returns in a variable:
apple = StockReturns[,"AAPL"]
CumulAAPL = sum(apple)
AvgDaily = mean(apple)
StdAAPL = SD(apple)
3. mytickers = c("SPY", "AAPL", "YHOO")
Yahoo = StockReturns[,,"YHOO"]
CumulYHOO = sum(Yahoo)
AvgDailyY = mean(Yahoo)
StdYHOO = SD(Yahoo)
<br>
...
Rows = nrow(transposedData)
Columns = ncol(transposedData)

2. 
meanReturns = apply(transposedData,2,mean)
names(meanReturns) <- rownames(StockReturns)
pnl_plot(meanReturns)
...
**Your Answers here:**
1. dataset1.R - change startDate = 2005-01-01
